...
from others.other_components.daman.daman_networks.daman_blocks import UNetConvBlock2D, UNetUpSamplingBlock2D
from others.other_components.daman.daman_util.daman_metrics import jaccard, accuracy_metrics
...
class UNetDecoder2D(nn.Module):
    ...
    def __init__(self, in_channels=1, out_channels=2, feature_maps=64, levels=4, skip_connections=True,
                 group_norm=True):
...
class UNet2D(nn.Module):
    ...
    def __init__(self, in_channels=1, out_channels=2, feature_maps=64, levels=4, group_norm=False,
        ...
        self.save_model_path = './logs/'

        # contractive path
        self.encoder = UNetEncoder2D(in_channels, feature_maps=feature_maps, levels=levels, group_norm=group_norm)
        # expansive path
        self.decoder = UNetDecoder2D(in_channels, out_channels, feature_maps=feature_maps, levels=levels,
                                     skip_connections=skip_connections, group_norm=group_norm)
    ...
    # trains the network
    def train_net(self, train_loader, test_loader, loss_fn, lr=1e-3, step_size=1, gamma=1, epochs=100, test_freq=1,
        ...
        for epoch in range(epochs):
            ...
            if (epoch + 1) % 5 == 0 or epoch == epochs - 1:
                t = "unet-"
                model_save_name = self.save_model_path + t + str(epoch + 1) + ".pt"
                torch.save(self.state_dict(), model_save_name)
                print("saving model")
...
def unet_from_encoder_decoder(encoder, decoder):
    net = UNet2D(in_channels=encoder.in_channels, out_channels=decoder.out_channels, feature_maps=encoder.feature_maps,
